...
> {
  ...
  private _tabs = [
    ...
    { title: '群成员', key: E_MSG_TYPE.MEMBER },
  ...
  private onChangeTab(tab: any, keyword?: string) {
    ...
    switch (tab.key) {
      ...
      case E_MSG_TYPE.MEMBER:
        return this.getMembers(tab.key, '')
  ...
  private onRefresh(done: Function) {
    ...
    switch (param.type) {
      ...
      case E_MSG_TYPE.DATE:
        ...
        if (param.type === E_MSG_TYPE.MEMBER) {
          data.fromChatId = param.fromChatId
        }
  ...
  private onSureSearch() {
    ...
    switch (param.type) {
      ...
      case E_MSG_TYPE.DATE:
        ...
        if (param.type === E_MSG_TYPE.MEMBER) {
          if (!param.fromChatId) {
            return this.getMembers(param.type, param.keyword || '')
          }
          data.fromChatId = param.fromChatId
        }
  ...
  private onGetMsgsByMember(itm: any = {}) {
    const chatId = itm.chat_id || ''
    const { loading, param } = this.state
    if (loading || !chatId) {
      return
    }
    ...
    this.setState({
      hasData: false,
      msgs: [],
      param: { ...param, fromChatId: chatId, keyword: '' }
    })

    this.getMembers(param.type, '')
    this.getMsgs({ type: param.type, page: 1, fromChatId: chatId, keyword: '' })
  }
  ...
  private async getMsgs(params: any, callback?: Function) {
    ...
    const result = await this.props.contact.getGroupHisMsgs(param)
  ...
  private async getMembers(type: E_MSG_TYPE, keyword: string = '') {
    const { initMembers, groupId, groupChatId } = this.state
    const { mbs = [], adminId = '' } = initMembers
    let iniMbs = serialize.copy(mbs)
    let admId = adminId

    const params = { keyword, type }
    this.setState({ memberLoading: true, loading: true })

    // 获取群成员
    if (iniMbs.length === 0) {
      await this.props.contact.getGroupById(groupId, groupChatId)
      const { members = [], admin_id = '' } =
        this.props.contact.groupDetail || {}
      iniMbs = serialize.copy(members)
      admId = admin_id
      this.setState({
        initMembers: { mbs: serialize.copy(members), adminId: admin_id }
      })
    }

    // 筛选
    if (params.keyword) {
      iniMbs = iniMbs.filter((m: any) =>
        (m.realname || '').includes(params.keyword)
      )
    }

    // 排序
    const result: any[] = zhSort({ key: 'realname', data: iniMbs })
    const admin = iniMbs.find((m: any) => m.id === admId)
    if (admin) {
                  ...
                  >
                    ...
                    >
                      ...
                      <div className="pr">
                        ...
                        {isSelf ? null : (
                          <div
                            className={`${chatStyle.fromSend} db mb10 tes fs12`}
                          >
  ...
  private onGoBack() {
    const { param, groupId } = this.state

    if (param.type === E_MSG_TYPE.MEMBER && param.fromChatId) {
      return this.setState({ param: { ...param, fromChatId: '', keyword: '' } })
    }
    ...
    // this.props.history.replace(`/group-set/${groupId}/${param.toChatId}`)
    Header.goBack()
  }
    ...
    return (
      <div className="wp100 hp100 pr bsb pt50 fs0 bg-f0f0f0 oh">
        <Header onClickLeft={this.onGoBack}>
          {groupName ? `${groupName}的聊天记录` : '未设置'}
        </Header>
        {param.type === E_MSG_TYPE.ALL_FILES ||
        (param.type === E_MSG_TYPE.MEMBER && param.fromChatId) ||
        (param.type === E_MSG_TYPE.DATE && param.date) ? (
          <div className={`${style.groupChatMsgs} wp100 hp100 bsb pr oh`}>
            {this.msgWrap()}
          </div>
        ) : (
          <div className={`${style.groupChatMsgs} wp100 pt40 hp100 bsb pr oh`}>
            ...
            >
              {this.msgWrap()}
              <div
                key={E_MSG_TYPE.MEMBER}
                className="member wp100 hp100 oh bsb pr"
              >
                <div className="search bg-fff">
                  <Search
                    value={param.keyword}
                ...
                <Loading spinning={memberLoading} className="oay sb">
                  <LetterSort
                          ...
                          <div className="wp100 bg-fff">
                            {m.data.map((d: any) => (
                              <div
                                ...
                                onClick={() => this.onGetMsgsByMember(d)}
                              >
                                <Image
                                  className="icon"
                                  url={d.head_img}
